...
bool validate_rgb_value(const char *value) {
  ...
  // Check if the value is in the format r,g,b and each component is within the
  // 0-255 range
...
void edit_config(char *setting, char *value, char *config_path) {
  ...
  if (!validate_rgb_value(value)) {
    fprintf(stderr,
            "Invalid value: %s. Expected format: r,g,b with each component "
            "between 0 and 255.\n",
            value);
    return;
  }
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (strncmp(line, setting, strlen(setting)) == 0 &&
        line[strlen(setting)] == '=') {
      fprintf(temp_fp, "%s=%s\n", setting, value);
      found = 1;
    } else {
