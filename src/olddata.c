...
char *clean_string(char *string, const char *prefix, const char *suffix) {
  ...
  // After potentially trimming the suffix, set the `end` pointer again to the
  // last character of the meaningful content.
...
char *get_info(DataPoint dp) {
  ...
  case COMPUTER: {
    char *name = get_from_file("/sys/devices/virtual/dmi/id/product_name", NULL,
                               "", "\n");
    char *version = get_from_file("/sys/devices/virtual/dmi/id/product_version",
                                  NULL, "", "\n");
  ...
  case CPU: {
    ...
    if (cpu && threads && freq) {
      char *finalcpu =
          malloc(strlen(cpu) + strlen(threads) + strlen(freq) + 10);
      if (finalcpu) {
        // Print to a formatted string
        snprintf(finalcpu, strlen(cpu) + strlen(threads) + strlen(freq) + 10,
                 "%s (%s) @ %sGHz", cpu, int_to_string(th),
                 double_to_string(gh, ".2f"));
        free(cpu);
        free(threads);
        free(freq);
        result = finalcpu;
  ...
  case MEMORY: {
    ...
    char *total_mem =
        get_from_file("/proc/meminfo", "MemTotal", "MemTotal:", " kB\n");
    if (total_mem) {
    ...
    char *free_mem =
        get_from_file("/proc/meminfo", "MemFree", "MemFree:", " kB\n");
    if (free_mem) {
      fr = extract_int(free_mem);
      free(free_mem);
    }
    char *buffer_mem =
        get_from_file("/proc/meminfo", "Buffers", "Buffers:", " kB\n");
    if (buffer_mem) {
      bu = extract_int(buffer_mem);
      free(buffer_mem);
    }
    char *sr_mem = get_from_file("/proc/meminfo", "SReclaimable",
                                 "SReclaimable:", " kB\n");
    if (sr_mem) {
    ...
    char *cached_mem =
        get_from_file("/proc/meminfo", "Cached", "Cached:", " kB\n");
    if (cached_mem) {
    ...
    if (num && denom) {
      ...
      if (result) {
        // Print to a formatted string
        snprintf(result, strlen(num) + strlen(denom) + 20, "%sMiB / %sMiB", num,
                 denom);
      }
      free(num);
      free(denom);
