...
void print_sysgrab(const Color *base_color, const Color *accent_color,
                   char **art, const size_t *max_line_len,
                   const size_t *line_count);
void print_line(const Color *base_color, const Color *accent_color,
                const size_t *max_line_len, char *art_string, char *info_type,
                char *info_string);
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, "hvb:a:", long_options,
                            &option_index)) != -1) {
    switch (opt) {
    case 'h':
      show_help(argv[0]);
  ...
  if (config != NULL) {
    ...
    for (int i = 0; i < config_count; i++) {
      if (strcmp(config[i].name, "base_color") == 0) {
        sscanf(config[i].value, "%hhu,%hhu,%hhu", &base_color.r, &base_color.g,
               &base_color.b);
      }
      if (strcmp(config[i].name, "accent_color") == 0) {
        sscanf(config[i].value, "%hhu,%hhu,%hhu", &accent_color.r,
               &accent_color.g, &accent_color.b);
      }
    }
...
void print_sysgrab(const Color *base_color, const Color *accent_color,
                   char **art, const size_t *max_line_len,
                   const size_t *line_count) {
  char *data_points[] = {"OS: ",    "Architecture: ", "Kernel: ", "Host: ",
                         "Shell: ", "Uptime: ",       "CPU: ",    "Memory: "};
  ...
  if (art != NULL) {
    ...
    if (username && hostname) {
      ...
      if (*line_count > 0)
        print_line(base_color, accent_color, max_line_len, art[0], username,
                   hostname);
      else
        print_line(base_color, accent_color, max_line_len, "", username,
                   hostname);
    ...
    for (DataPoint dp = OS; dp <= MEMORY; dp++) {
      ...
      if (info) {
        ...
        if (dp < *line_count) {
          print_line(base_color, accent_color, max_line_len, art[dp],
                     data_points[dp - 2], info);
        } else {
          // Otherwise print with no art
          print_line(base_color, accent_color, max_line_len, "",
                     data_points[dp - 2], info);
        }
      } else {
    ...
    if (username && hostname) {
      ...
      print_line(base_color, accent_color, max_line_len, NULL, username,
                 hostname);
    ...
    for (DataPoint dp = OS; dp <= MEMORY; dp++) {
      char *info = get_info(dp);
      if (info) {
        print_line(base_color, accent_color, max_line_len, NULL,
                   data_points[dp - 2], info);
      } else {
...
void print_line(const Color *base_color, const Color *accent_color,
  ...
  printf("\033[38;2;%d;%d;%dm", accent_color->r, accent_color->g,
         accent_color->b);
